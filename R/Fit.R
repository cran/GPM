...
#' @description Fits a Gaussian process (GP) to a set of simulation data as described in \code{reference 1}. Both the inputs and outputs can be multi-dimensional. The outputs can be noisy.
...
#' @param CorrType The type of the correlation function of the GP model. Choices include \code{'G'} (default), \code{'PE'}, \code{'LBG'}, and \code{'LB'}. See the \code{references} for the details. For smooth (or analytic) functions, choose either
...
#' @param Nopt The number of times the log-likelihood function is optimized when Eps[1] is used to constraint the smallest eigen value that the correlation matrix is allowed to have. Higher \code{Nopt} will increase fitting time as well as the chances of finding the global optimum.
...
#' @param Criterion Either \code{LOOCV} or \code{LogL}. Criterion used to determine the most optimum constraint (i.e. an element of \code{Eps}).
...
Fit <-  function(X, Y, CorrType = 'G', Eps = 10^(seq(-1, -12)),Nopt = 5,TraceIt = 0,
                 ...
                 Criterion = 'LOOCV', StopFlag = 0, Progress = 0) {
  ...
  if (is.character(Criterion)){
    if(Criterion != 'LOOCV' && Criterion != 'NLogL'){
      stop('Criterion should be either LOOCV or NLogL.')
    }
  } else{
    stop('Criterion should be a string (either LOOCV or NLogL).')
  }
  ...
  if (is.null(LowerBound)){
    if (Progress != 0){
      cat('     The default values are used for initializing the lower bounds on hyper-parameters.\n')
    }
    if (CorrType == 'G') {LowerBound = rep(x = Omega_min_default, times = p)}
    else if (CorrType == 'PE') {LowerBound = c(rep(x = Omega_min_default, times = p), 1)}
    else if (CorrType == 'LBG') {LowerBound = c(rep(x = Omega_min_default, times = p), Zeta_min_default)}
    else if (CorrType == 'LB') {LowerBound = c(rep(x = Omega_min_default, times = p), Zeta_min_default, Gamma_min_default)}
  } else {
    if (!is.vector(LowerBound)) LowerBound = as.vector(LowerBound)
    if (CorrType == 'G' && length(LowerBound) != p){
      stop(paste('     The size of the provided LowerBound is incorrect! It should be', toString(p)))}
    else if (CorrType == 'PE' && length(LowerBound) != (1 + p)){
      stop(paste('     The size of the provided LowerBound is incorrect! It should be', toString(p + 1)))}
    else if (CorrType == 'LBG' && length(LowerBound) != (1 + p)){
      stop(paste('     The size of the provided LowerBound is incorrect! It should be', toString(p + 1)))}
    else if (CorrType == 'LB' && length(LowerBound) != (2 + p)){
      stop(paste('     The size of the provided LowerBound is incorrect! It should be', toString(p + 2)))}
    if (any(LowerBound[1:p] < -10)){
      warning(paste('The first ', toString(p), ' values in LowerBound are recommended to be larger than -10.'))
    }
  }

  if (is.null(UpperBound)){
    if (Progress != 0){
      cat('     The default values are used for initializing the upper bounds on hyper-parameters.\n')
    }
    if (CorrType == 'G') {UpperBound = rep(x = Omega_max_default, times = p)}
    else if (CorrType == 'PE') {UpperBound = c(rep(x = Omega_max_default, times = p), 2)}
    else if (CorrType == 'LBG') {UpperBound = c(rep(x = Omega_max_default, times = p), Zeta_max_default)}
    else if (CorrType == 'LB') {UpperBound = c(rep(x = Omega_max_default, times = p), Zeta_max_default, Gamma_max_default)}
  } else {
    if (!is.vector(UpperBound)) UpperBound = as.vector(UpperBound)
    if (CorrType == 'G' && length(UpperBound) != p){
      stop(paste('     The size of the provided UpperBound is incorrect! It should be', toString(p)))}
    else if (CorrType == 'PE' && length(UpperBound) != (1 + p)){
      stop(paste('     The size of the provided UpperBound is incorrect! It should be', toString(p + 1)))}
    else if (CorrType == 'LBG' && length(UpperBound) != (1 + p)){
      stop(paste('     The size of the provided UpperBound is incorrect! It should be', toString(p + 1)))}
    else if (CorrType == 'LB' && length(UpperBound) != (2 + p)){
      stop(paste('     The size of the provided UpperBound is incorrect! It should be', toString(p + 2)))}
  }
  if (CorrType == 'PE' && (LowerBound[p + 1] < 1 || UpperBound[p+ 1] > 2)){
    stop(paste('     The exponent in PE should be between ', toString(c(1, 2)), ' (inclusive).'))
  } else if (CorrType == 'LBG' && (LowerBound[p + 1] <= 0 || UpperBound[p+ 1] > Zeta_max_default)){
    stop(paste('     Beta in LBG should be between ', toString(c(0, 1)), ' (exclusive).'))
  } else if (CorrType == 'LB' && (LowerBound[p+1]<=0 || UpperBound[p+1]>Zeta_max_default || LowerBound[p+2]<Gamma_min_default || UpperBound[p+2]>Gamma_max_default)){
  ...
  for (i in 1:Tries){
    ...
    for (j in 1: Nopt_i){
      temp <- stats::optim(Var_int[j, ], NLogL, gr = NULL, XN, YN, CorrType, Eps_i, Fn, k, q,
    ...
    R <- CorrMat(XN, XN, CorrType, Var_unique[1, ])
    ...
    if (CorrType == 'PE' || CorrType=='G'){
      ...
      for (ii in 1:q){
        temp <- solve(diag(diag(t(inv_L)%*%inv_L)), Rinv_YN[, ii, drop = FALSE] - Rinv_Fn%*%B[ii])
        LOOCV[ii] <- mean(abs(temp))
      }
    } else {
      for (ii in 1:q){
        temp <- solve(diag(diag(t(inv_L)%*%inv_L)), Rinv_YN[, ii, drop = FALSE])
        LOOCV[ii] <- mean(abs(temp))
      }
    }
  ...
  if (Criterion == 'LOOCV'){
    ID <- which.min(lapply(OptimHist$LOOCV, sum))
  } else {
  ...
  ## Save the results
  Model <- NULL
  Model$CovFunc <- list('CorrType' = CorrType, 'Parameters' = Parameters, 'LowerBound' = LowerBound, 'UpperBound' = UpperBound)
  Model$Data <- list('XN' = XN, 'k' = k, 'Xmin' = Xmin, 'Xmax' = Xmax,  'YN' = YN, 'q' = q, 'Yrange' = Yrange, 'Ymin' = Ymin)
  Model$Details <- list('Fn' = Fn, 'L' = L, 'Raw_MinEig' = Raw_MinEig, 'Nug_opt' = Nug_opt, 'LOOCV' = LOOCV, 'Cost' = Cost, 'MinNLogL' = OptimHist$Obj_Fun_best[ID])
